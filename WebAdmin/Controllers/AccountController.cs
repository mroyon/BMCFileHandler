...
namespace WebAdmin.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        /// <summary>
        /// GetQRCodeFromPACILogin
        /// </summary>
        /// <param name="request"></param>
        /// <returns></returns>
        [HttpPost]
        [AllowAnonymous]
        public async Task<IActionResult> GetQRCodeFromPACILogin([FromBody] PACIAuthRequestEntity request)
        {
            if (!ModelState.IsValid) { return BadRequest(ModelState); }


            PaciKeyParamsValuesEntity objKeyParam = new PaciKeyParamsValuesEntity();
            objKeyParam.ResponseCode = string.Empty;
            objKeyParam.CivilID = string.Empty;
            objKeyParam.WebSessionID = request.HUBSessionid;
            objKeyParam.SignalRConnectionID = request.HUBCnnectionId;
            objKeyParam.SignInRequestID = -99;
            objKeyParam.IsQRAuth = true;
            string strKeyParam = JsonConvert.SerializeObject(objKeyParam);

            AppConfig.EncryptionHandler.EncryptionHelper objEnc = new AppConfig.EncryptionHandler.EncryptionHelper();
            request.KeyParam = objEnc.encryptSimple(strKeyParam);

            request.SignalRContextName = _hostingDomainSettings.SignalRContextNameQR;
            request.CallingUrl = _hostingDomainSettings.CompleteDomainURL;
            request.coreornocore = true;
            request.SignalRMethodName = "PostAuthenticationProcessCallQR";


            request.servicename = request.servicename + " : QRCode";
            request.ServiceDescriptionEN = "National military service";
            request.ServiceDescriptionAR = "الخدمة العسكرية الوطنية";

            request.AuthenticationReasonEn = "National military service: Login";
            request.AuthenticationReasonAr = "الخدمة العسكرية الوطنية: تسجيل الدخول";


            await _auth_UseCase.GetQRCodeFromPACIToAuthenticate(new Auth_Request(request), _auth_UsePresenter);
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> LoginQR([FromBody] owin_userEntity request)
        {
            var returnUrl = request.ReturnUrl;
            ViewData["ReturnUrl"] = returnUrl;
            ...
            request.emailaddress = request.userprofilephoto;
            await _auth_UseCase.LoginWebPACIRequest(new Auth_Request(request), _auth_UsePresenter);
            return Json(_auth_UsePresenter.ContentResult);
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> GenerateQr(string returnUrl)
        {
            return Json(new AjaxResponse("200", _sharedLocalizer["VERIFY"].Value, CLL.LLClasses._Status._statusSuccess, CLL.LLClasses._Status._titleInformation, "/"));
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> GetCivilIDValidated([FromBody] owin_userEntity request)
        {
            ModelState.Remove("passwordquestion");
            ModelState.Remove("passwordkey");
            ModelState.Remove("passwordvector");
            ...
            await _auth_UseCase.GetCivilIDValidated(new Auth_Request(request), _auth_UsePresenter);
            return Json(_auth_UsePresenter.ContentResult);

        }

        /// <summary>Civil Id authenticated using PACI API.</summary>
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> PaciAuthRequestByCivilId([FromBody] PACIAuthRequestEntity request)
        {
            if (!ModelState.IsValid) { return BadRequest(ModelState); }
            ...
            objKeyParam.WebSessionID = request.HUBSessionid;
            objKeyParam.SignalRConnectionID = request.HUBCnnectionId;
            objKeyParam.CivilID = request.civilid;
            objKeyParam.SignInRequestID = -99;
            objKeyParam.IsQRAuth = true;
            string strKeyParam = JsonConvert.SerializeObject(objKeyParam);
            ...
            request.SignalRContextName = _hostingDomainSettings.SignalRContextNameCivil;
            request.CallingUrl = _hostingDomainSettings.CompleteDomainURL;
            request.coreornocore = true;
            request.SignalRMethodName = "PostAuthenticationProcessCallCivil";


            request.servicename = request.servicename + " : CivilID Auth";
            request.ServiceDescriptionEN = "National military service";
            request.ServiceDescriptionAR = "الخدمة العسكرية الوطنية";
            ...
            await _auth_UseCase.PaciAuthRequestByCivilId(new Auth_Request(request), _auth_UsePresenter);
            return Json(_auth_UsePresenter.Result);
        }
    }
}