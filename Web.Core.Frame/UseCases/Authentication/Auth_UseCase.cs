    ...
    public sealed class Auth_UseCase : IAuth_UseCase
    {
        ...
        public async Task<bool> LoginWebPACIRequest(Auth_Request message, IOutputPort_Auth<Auth_Response> outputPort)
        {
            ...
            try
            {
                ...
                var CheckUser = _userManager.CheckUserExists(objUser.emailaddress, objUser.password, hrprofileJsonString);
                var returnUrl = message.Obj_owin_user.ReturnUrl;
                ...
                if (user != null)
                {
                    ...
                    result = await _signInManager.PasswordSignInAsync(user, user.pkeyex.GetValueOrDefault().ToString(), 
        ...
        public async Task<bool> PaciAuthRequestByCivilId(Auth_Request message, IOutputPort_Auth<Auth_Response> outputPort)
        {
            try
            {
                //CancellationToken cancellationToken = new CancellationToken();
                ...
                //Obj_owin_user.pkeyex = long.Parse(message.Obj_PACIAuthRequest.civilid);
                ...
                //        tran_pacisigninrequestinfoSerial = objRet.data.tran_pacisigninrequestinfoSerial,
